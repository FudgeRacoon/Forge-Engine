...
namespace Forge {
	namespace Graphics
	{
		 ...
		 * @author Karim Hisham.
		 */
		class FORGE_API AbstractHardwareBuffer
		{
		protected:
			union Handle
			{
		...
		public:
			 ...
			 */
			AbstractHardwareBuffer(Size size, UsageModeMask usage_mode_mask, Bool is_shadowed);
		...
		public:
			/**
			 * @brief Gets the native handle of the hardware buffer.
			 * 
		...
		public:
			 ...
			 * than the size of the buffer or if the offset is beyond the buffer
			 * region.
			 */
		...
		FORGE_FORCE_INLINE AbstractHardwareBuffer::AbstractHardwareBuffer(Size size, UsageModeMask usage_mode_mask, Bool is_shadowed)
			: m_size(size), m_lock_offset(0), m_lock_length(0), m_is_locked(false), m_is_shadowed(is_shadowed), m_usage_mode_mask(usage_mode_mask)
		{
		...
		FORGE_FORCE_INLINE AbstractHardwareBuffer::Handle AbstractHardwareBuffer::GetNativeHandle(Void)
		{
			return m_handle;
		}
